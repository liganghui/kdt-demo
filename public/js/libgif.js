...
;(function (root, factory) {
  ...
  var SuperGif = function (opts) {
    ...
    if (typeof options.auto_play == 'undefined')
      options.auto_play =
        !gif.getAttribute('rel:auto_play') || gif.getAttribute('rel:auto_play') == '1'
    ...
    var doGCE = function (gce) {
        ...
        // 设置目标帧率
        var targetFps = 30;
        var targetDelay = 1000 / targetFps; // 33.33ms for 30fps
        // 只有当 delay 大于目标帧率延迟时，才进行限制
        if (gce.delayTime > targetDelay) {
          delay = targetDelay; // 限制为 33.33ms
        } else {
          delay = gce.delayTime; // 保持原来的 delay
        }
